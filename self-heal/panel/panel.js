...
(function () {
  ...
  function renderReport() {
    ...
    h += '<h3 style="margin-top:14px">Export (T6.2) — the bridge to the installable-plugin phase</h3>' +
         '<div class="mut">Playwright stub is TEXT GENERATION ONLY — there is no Node in this environment to run or type-check it, so it is tagged <code>asserted</code>, never <code>measured</code>.</div>' +
         '<select id="exportTest" class="btn" style="margin-top:6px"></select> ' +
         '<button class="btn" id="exportJson">⬇ export OpenTest.ai JSON</button> ' +
         '<button class="btn" id="exportPw">⬇ generate Playwright stub (.spec.ts)</button>' +
         '<pre id="exportOutput" class="hide" style="max-height:240px;overflow:auto;margin-top:8px"></pre>';
    root.innerHTML = h;
    const sel = $('#exportTest');
    (state.suite ? state.suite.tests : []).forEach(t => { const o = document.createElement('option'); o.value = t.id; o.textContent = t.id + ' — ' + t.title; sel.appendChild(o); });
    $('#exportJson').addEventListener('click', () => exportSuiteJson(sel.value));
    $('#exportPw').addEventListener('click', () => exportPlaywrightStub(sel.value));
  }

  // ---- export (T6.2): OpenTest.ai JSON (the suite's own native shape — no transform needed) + a
  // generated Playwright .spec.ts stub from _anchor.bestLocator. Both are DOWNLOADS (Blob + <a
  // download>, client-side only — no server, no filesystem write outside the browser's own dialog).
  ...
  function exportSuiteJson(testId) {
    if (!state.suite) { chatMsg('agent', 'No suite authored yet.'); return; }
    const test = testId ? state.suite.tests.find(t => t.id === testId) : null;
    const payload = test || state.suite;
    const text = JSON.stringify(payload, null, 2);
    downloadText((test ? test.id : 'suite') + '.opentest.json', text);
    const out = $('#exportOutput'); out.textContent = text; out.classList.remove('hide');
    chatMsg('agent', 'Exported OpenTest.ai JSON for <b>' + esc(test ? test.id : 'the whole suite') + '</b> (downloaded).');
  }
  // role=button[name='Sign in'] -> page.getByRole('button', { name: 'Sign in' }); anything else
  // (testid/#id/[name=]) is a real CSS selector already, so page.locator(...) is correct as-is.
  function toPlaywrightLocator(anchor) {
    const bl = anchor && anchor.target && anchor.target.bestLocator;
    if (!bl) return null;
    const m = /^role=([a-z]+)\[name='(.*)'\]$/.exec(bl);
    if (m) return "page.getByRole('" + m[1] + "', { name: '" + m[2].replace(/'/g, "\\'") + "' })";
    return "page.locator('" + bl.replace(/'/g, "\\'") + "')";
  }
  function exportPlaywrightStub(testId) {
    if (!state.suite) { chatMsg('agent', 'No suite authored yet.'); return; }
    const test = state.suite.tests.find(t => t.id === testId) || state.suite.tests[0];
    if (!test) { chatMsg('agent', 'No test to export.'); return; }
    const lines = [];
    lines.push('import { test, expect } from \'@playwright/test\';', '');
    lines.push('// GENERATED by the QA Agent panel (P2 T6.2) from _anchor.bestLocator — TEXT ONLY,');
    lines.push('// never run/type-checked here (no Node in this environment). Tag: asserted.');
    lines.push('test(' + JSON.stringify(test.title) + ', async ({ page }) => {');
    let weakAnchorNote = false;
    test.steps.forEach(s => {
      const loc = toPlaywrightLocator(s._anchor);
      if (!loc && s.action !== 'assert') weakAnchorNote = true;
      if (s.action === 'fill' && loc) lines.push('  await ' + loc + '.fill(' + JSON.stringify(s.value || '') + ');');
      else if (s.action === 'click' && loc) lines.push('  await ' + loc + '.click();');
      else if (s.action === 'assert') lines.push('  await expect(page.getByText(' + JSON.stringify(s.target || '') + ')).toBeVisible();');
      else lines.push('  // TODO: no strong locator captured for "' + (s.description || s.target || s.action) + '" — re-record with a stable anchor');
    });
    lines.push('});');
    if (weakAnchorNote) lines.push('', '// NOTE: one or more steps had no CSS-resolvable anchor (role+name-only or no-anchor) — see TODOs above.');
    const text = lines.join('\n');
    downloadText(test.id + '.spec.ts', text);
    const out = $('#exportOutput'); out.textContent = text; out.classList.remove('hide');
    chatMsg('agent', 'Generated a Playwright stub for <b>' + esc(test.id) + '</b> (downloaded, <b>asserted</b> — not run).');
  }
